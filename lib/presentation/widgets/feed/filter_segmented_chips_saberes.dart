...
class FilterSegmentedChipsSaberes extends StatelessWidget {
  final bool isLoggedIn;
  final VoidCallback onLoginTap;

  const FilterSegmentedChipsSaberes({super.key, required this.isLoggedIn, required this.onLoginTap});

  @override
  Widget build(BuildContext context) {
    final filtro = context.select<SaberesFeedProvider, FeedFilter>((p) => p.filtro);
          ...
          onSelected: enabled
              ? (_) {
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    final p = context.read<SaberesFeedProvider>();
                    p.setFiltro(value);
                  });
                }
              : null,
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final chips = <Widget>[
            ...
            if (isLoggedIn) chip('Mis saberes', FeedFilter.mis) else ActionChip(
